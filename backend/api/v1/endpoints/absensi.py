...
def get_all_absensi(
    ...
    query = query.order_by(Absensi.tanggal.desc(), Absensi.jam_masuk.desc())
    
    # Get total count
    total = query.count()
    
    # Apply pagination
    absensi_list = query.offset(skip).limit(limit).all()
    
    # Convert to response format with pegawai name
    result = []
    for a in absensi_list:
        data = {
            "id": a.id,
            "id_pegawai": a.id_pegawai,
            "pegawai_nama": a.pegawai.nama if a.pegawai else None,
